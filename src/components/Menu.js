  ...
  render() {
    const list = Dataset.map((d, i) => {
      const elms = d.data.map((elm, j) => {
        let url = `${elm.author.split(" ").join("_")}-${elm.topic.replace(
          / /g,
          "-"
        )}-${d.year}`;
        url = url.toLowerCase();
        return (
          <li
            key={j}
            className={url === this.props.url ? "text-highlight" : ""}
    ...
    return (
      <div>
        <div onClick={this.hideMenu} className="menu-close menu-icon">
          <Icon name="close" size="lg" /> Close
        </div>
        <div>{list}</div>
      </div>
    );
