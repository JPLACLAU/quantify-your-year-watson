  ...
  render() {
    ...
    const consecInactiveList = consec.inactive.list.map((item, i) => (
      <span key={i}>
        ...
        {i < consec.inactive.list.length - 1 && ", "}
      </span>
    ));
      ...
      <div className={`col-xs-12 stats ${color}`}>
        ...
        {unit === "day" && (
          <div className="show-inline">
            ...
            {` `}
            <ii>
              {(stats.active[unit] / 10).toFixed(1)} <l>days/week</l>
            </ii>
            {` `}and{" "}
            <ii>
              {(stats.active[unit] / 2.5).toFixed(1)} <l>days/month</l>
            </ii>
            .
          </div>
        )}
        {consec.active.count > 1 && (
          <div>
