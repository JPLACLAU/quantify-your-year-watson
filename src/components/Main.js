...
class Visualization extends Component {
  ...
  render() {
    ...
    const { year, color, abbr, alt_abbr, hasNegative, isReverse, decimal } =
      setting;
    ...
    return (
      <div className="row unit-selection">
        ...
        <div className="col-xs-12 visible-xs-block visible-sm-block slider">
          <Icon name="arrow-circle-left" /> Slide calender{" "}
        ...
        <Calendar
          ...
          calendar={calendar}
        />
        <Max {...setting} unit={unit} calendar={calendar} />
        {setting.considerFrequency && (
          <Stats {...setting} data={data} unit={unit} />
        )}
      </div>
    );
  }
